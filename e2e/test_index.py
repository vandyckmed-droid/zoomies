...
class BrowserTest(unittest.TestCase):
    ...
    def test_detail_panel_risk_equivalent_size(self):
        """Risk-equivalent size is 10 / dailyVol rounded to the nearest $100,
        floored at $100, and omitted entirely when there is no usable
        volatility to divide by.

        Asserted against a synthetic universe rather than committed data for
        the same reason the rank-change fixture is: every real name's
        volatility changes on the next rebuild, so pinning an exact dollar
        figure to today's data would test the data, not the arithmetic.

        The missing case is modelled as an *unscored* name with no cached
        volatility, which is the real shape of that state -- all 23 unscored
        names in the committed build have dailyVol null. A scored name with
        null dailyVol is not reachable (the score is computed from it) and
        would already break the existing "Daily volatility" row.
        """
        work = pathlib.Path(tempfile.mkdtemp())
        self.addCleanup(shutil.rmtree, work, True)
        for name in ("index.html", "returns.js"):
        ...
        rep = report()
        template = rep["universe"][0]
        # (symbol, score, dailyVol) -- 10/0.02 = $500 exactly; 10/0.4 = $25,
        # which rounds to $0 and must be lifted to the $100 floor; 10/0.01 =
        # $1000 on a name with no score at all, since the figure does not
        # depend on the 12-1 score.
        fixture = [
            ("NORM", 5.0, 0.02),
            ("FLOOR", 4.0, 0.4),
            ("NOSCORE", None, 0.01),
        ...
        rows = []
        for symbol, score, daily_vol in fixture:
            row = dict(template)
            row.update({
                "symbol": symbol, "name": symbol + " Corp",
                "score": score, "dailyVol": daily_vol,
                "lastClose": None, "lastDate": None,
            })
            rows.append(row)
        rep["universe"] = rows
        ...
        label = "Risk-equivalent size"

        def check(symbol, dollars):
            stats = self._open_by_symbol(page, symbol)
            self.assertIn(label, stats, "%s should show the row" % symbol)
            value = stats[label]
            self.assertTrue(
        ...
        # Unscored names are hidden by the default MIN SCORE of 1, so clear it
        # to reach the two that carry no score.
        page.fill("#f-score", "")
        page.wait_for_timeout(200)

        check("NOSCORE", "$1000")

        # No usable volatility -- the row is absent rather than showing a
