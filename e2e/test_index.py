...
class BrowserTest(unittest.TestCase):
    ...
    def test_score_trend_fetches_only_the_opened_ticker(self):
        """Same on-demand shape as the pair matrix, for the same reason: a
        page load only ever looks at one or two detail panels, not all 1,000
        tracked names, so opening one should fetch exactly one history shard.

        Uses a synthesized shard rather than whichever ticker the committed
        data currently ranks first: that name's actual history depth is not
        an invariant of the system (a recent listing could have too few
        valid snapshots for a sparkline), so asserting on it would tie this
        test's pass/fail to today's live rankings instead of the code.
        """
        work = pathlib.Path(tempfile.mkdtemp())
        self.addCleanup(shutil.rmtree, work, True)
        for name in ("index.html", "returns.js"):
            shutil.copy(ROOT / name, work / name)
        shutil.copytree(ROOT / "data" / "history", work / "data" / "history")

        rep = report()
        (work / "scores.js").write_text(
            "// Generated by build.py -- do not edit.\nconst REPORT = %s;\n"
            % json.dumps(rep, indent=1))
        target = rep["universe"][0]["symbol"]
        points = [["2026-07-%02d" % d, 1.0 + d * 0.01, 30 - d] for d in range(1, 6)]
        (work / "data" / "history" / (target + ".js")).write_text(
            "HISTORY[%s]=%s;\n" % (json.dumps(target), json.dumps(points)))

        page = self.page(url="file://%s/index.html" % work)
        requests = []
        page.on("request", lambda req: requests.append(req.url)
                if "/data/history/" in req.url else None)
        ...
        page.wait_for_selector("#overlay:not([hidden])")
        page.wait_for_function(
            "document.getElementById('ov-history').innerHTML !== ''", timeout=10000)

        self.assertEqual(len(requests), 1,
                          "opening one detail panel should fetch exactly one "
                          "history shard, not %r" % requests)
        self.assertEqual(requests[0].rsplit("/", 1)[-1], target + ".js")
        self.assertEqual(page.inner_text("#ov-history .spark-rank"), "5D: #29 → #25")
        self.assertEqual(page.eval_on_selector_all("#ov-history polyline", "e => e.length"), 1)
        self.assertEqual(page.errors, [])

    def test_missing_history_shard_leaves_correlations_unaffected(self):
        """History and returns are deliberately separate loading-state
        tracks, even though both are per-ticker shards -- a 404 on one must
        not touch the other. Force a 404 by deleting one starred name's
        history shard while leaving its returns data (returns.js) intact,
        and confirm the correlation panel (a different feature, different
        loading state) still renders normally. Also reopens the same panel a
        second time, to catch the shape of bug that shipped once already
        with the returns shards: a permanently-failed fetch being retried
        on every render instead of remembered as settled.
        """
        work = pathlib.Path(tempfile.mkdtemp())
        self.addCleanup(shutil.rmtree, work, True)
        for name in ("index.html", "returns.js"):
        ...
        (work / "scores.js").write_text(
            "// Generated by build.py -- do not edit.\nconst REPORT = %s;\n"
            % json.dumps(rep, indent=1))
        victim = rep["universe"][0]["symbol"]
        (work / "data" / "history" / (victim + ".js")).unlink()
        ...
        self.assertEqual(page.inner_html("#ov-history"), "",
                          "a missing history shard should leave the panel empty, not broken")
        self.assertNotIn("loading", page.inner_text("#ov-corr").lower())
        self.assertGreater(len(page.inner_text("#ov-corr")), 0,
                            "correlations should still render despite the missing history shard")

        page.click("#ov-close")
        page.click("#rows tr:first-child td.ticker")
        page.wait_for_timeout(300)
        self.assertEqual(len(requests), 1,
                          "a settled (failed) history fetch must not be retried on reopen")
        self.assertEqual(page.errors, [])
